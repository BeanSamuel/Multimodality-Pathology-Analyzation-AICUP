...
from model import BertForSequenceClassification, BertConfig, WEIGHTS_NAME, CONFIG_NAME,BertForTokenClassification
...
def main(i):
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default='D:\\NCU\\AICUP\\Multimodality\\Training_Dataset', type=str,
                        help="The input data dir. Should contain the .tsv files (or other data files) for the task.")
    parser.add_argument("--bert_model", default='D:\\NCU\\AICUP\\Multimodality\\OurModel\\pre-trained BERT', type=str,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                        "bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, "
                        "bert-base-multilingual-cased, bert-base-chinese.")
    parser.add_argument("--task_name", default='Multi', type=str,
                        help="The name of the task to train.")
    parser.add_argument("--output_dir", default='D:\\NCU\\AICUP\\Multimodality\\OurModel\\CM-BERT_output', type=str,
    ...
    #TODO
    df = pd.read_csv(args.data_dir+'\\training_datalist.csv')
    df['relative_path'] = df['ID'].astype(str)+'.wav'
    df['Disease category'] = df['Disease category'] - 1
    train_dataset, valid_dataset = train_test_split(df, random_state=777, train_size=0.8)
    train_dataset = train_dataset.reset_index(drop=True)
    valid_dataset = valid_dataset.reset_index(drop=True)
    print(train_dataset.head())
    if args.do_train:    
        train_examples = processor.get_train_examples(train_dataset)
    ...
    for i in train_dataset['relative_path']:
        sig, sr = torchaudio.load(args.data_dir+'\\training_voice_data\\'+i)
        sig,sr = AudioUtil.pad_trunc((sig,sr), 5000)
        train_audio.append(sig)

    train_audio = torch.stack(train_audio,dim=0)
    # print(train_audio.shape)
    
    for i in valid_dataset['relative_path']:
        sig, sr = torchaudio.load(args.data_dir+'\\training_voice_data\\'+i)
        sig,sr = AudioUtil.pad_trunc((sig,sr), 5000)
        valid_audio.append(sig)
    valid_audio = torch.stack(valid_audio,dim=0)
    ...
    if args.do_train:
        ...
        print("all_train_audio: ", all_train_audio.shape)
        print("all_input_ids: ",all_input_ids.shape)
        print("all_input_mask: ",all_input_mask.shape)
        ...
        # for a,b,c,d,e in train_data:
        #     print("all_input_ids",a)
        #     print("all_input_mask",b)
        #     print("all_segment_ids",c)
        #     print("all_train_audio",d)
        #     print("all_labels_ids",e)
        #     break
        train_sampler = RandomSampler(train_data)
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size)

        ## Evaluate for each epcoh
        eval_examples = processor.get_dev_examples(valid_dataset)
